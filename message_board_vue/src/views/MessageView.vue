<template>
    <div class="message-view">
        <div class="content-container">
            ...
            <!-- 回复列表 -->
            <div v-if="currentMessage && currentMessage.remark && currentMessage.remark.length > 0"
                        ...
                        <div v-for="reply in currentMessage.remark" :key="reply.rid" class="reply-card">
                            <div class="reply-header">
                                ...
                                <div class="reply-info">
                                    ...
                                    <span class="reply-time">{{ formatTime(new Date(reply.time)) }}</span>
                                </div>
                                <div v-if="isMyRemark(reply)" class="reply-actions">
                                    <el-button v-if="editingRemarkId !== reply.rid" text type="primary" size="small"
                                        :icon="Edit" @click="startEditRemark(reply)">
                                        编辑
                                    </el-button>
                            ...
                            <div v-if="editingRemarkId === reply.rid" class="reply-edit">
                                <Chat v-model="editingContent" placeholder="编辑回复..." :min-height="'100px'"
                                    :max-height="'200px'" />
            ...
            <!-- 暂无回复提示 -->
            <div v-else-if="currentMessage && (!currentMessage.remark || currentMessage.remark.length === 0)"
                class="empty-replies">
...
import { ChatLineRound, Star, ArrowLeft, Delete, Edit } from '@element-plus/icons-vue'
...
const handleReward = async () => {
    ...
    const isLiked = await messageStore.toggleLike({ mid: currentMessage.value.mid })
    if (isLiked !== null) {
